...
public class MainGUI {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		while (true) {
			window.goPan(1);
			String total;
			total = window.userInput();
			names = total.split("\n");
			window.goPan(2);
			int sessions = 0, groupSize = 0, blanks = 0;
			total = window.userInput();
			groupSize = Integer.parseInt(total.split("\t")[0]);
			sessions = Integer.parseInt(total.split("\t")[1]);
			...
			window.goPan(3);
			window.setText(groupFind(names.length, sessions, groupSize, blanks));
			window.userInput();
		}
	}
